...
static void proc_ldh(cpu_context *ctx) {
    if (ctx->current_inst->reg_1 == RT_A) {
        cpu_write_reg(ctx->current_inst->reg_1, bus_read(0xFF00 | ctx->fetch_data));
    } else {
...
static void proc_pop(cpu_context *ctx) {
    ...
    cpu_write_reg(ctx->current_inst->reg_1, n);

    if (ctx->current_inst->reg_1 == RT_AF) {
        cpu_write_reg(ctx->current_inst->reg_1, n & 0xFFF0);
    }
}
...
static void proc_sub(cpu_context *ctx) {
    u16 val = cpu_read_reg(ctx->current_inst->reg_1) - ctx->fetch_data;

    int z = val == 0;
    int h = ((int)cpu_read_reg(ctx->current_inst->reg_1) & 0xF) - ((int)ctx->fetch_data & 0xF) < 0;
    int c = ((int)cpu_read_reg(ctx->current_inst->reg_1)) - ((int)ctx->fetch_data) < 0;
...
static void proc_sbc(cpu_context *ctx) {
    ...
    cpu_write_reg(ctx->current_inst->reg_1, cpu_read_reg(ctx->current_inst->reg_1) - val);
    cpu_set_flags(ctx, z, 1, h, c);
}
...
static void proc_add(cpu_context *ctx) {
    u32 val = cpu_read_reg(ctx->current_inst->reg_1) + ctx->fetch_data;
    ...
    if (ctx->current_inst->reg_1 == RT_SP) {
        val = cpu_read_reg(ctx->current_inst->reg_1) + (int8_t)ctx->fetch_data;
    }

    int z = (val & 0xFF) == 0;
    int h = (cpu_read_reg(ctx->current_inst->reg_1) & 0xF) + (ctx->fetch_data & 0xF) >= 0x10;
    int c = (int)(cpu_read_reg(ctx->current_inst->reg_1) & 0xFF) + (int)(ctx->fetch_data & 0xFF) >= 0x100;

    if (is_16bit) {
        z = -1;
        h = (cpu_read_reg(ctx->current_inst->reg_1) & 0xFFF) + (ctx->fetch_data & 0xFFF) >= 0x1000;
        u32 n = ((u32)cpu_read_reg(ctx->current_inst->reg_1)) + ((u32)ctx->fetch_data);
        c = n >= 0x10000;
    }

    if (ctx->current_inst->reg_1 == RT_SP) {
        z = 0;
        h = (cpu_read_reg(ctx->current_inst->reg_1) & 0xF) + (ctx->fetch_data & 0xF) >= 0x10;
        c = (int)(cpu_read_reg(ctx->current_inst->reg_1) & 0xFF) + (int)(ctx->fetch_data & 0xFF) >= 0x100;
    }

    cpu_write_reg(ctx->current_inst->reg_1, val & 0xFFFF);
